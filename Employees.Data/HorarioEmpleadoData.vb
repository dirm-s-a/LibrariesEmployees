...
Public Partial Class HorarioEmpleadoData
	Inherits BaseData
...
#Region "Metodos Publicos"
	...
	Public Overloads Function Insert(ByVal Entity As HorarioEmpleado, Optional ByVal ThrowErr As Boolean = False) As Boolean
        Return MyBase.Insert(Entity.Table, Entity.DirtyFields)
    End Function

    Public Overloads Function Update(ByVal Entity As HorarioEmpleado, Optional ByVal ThrowErr As Boolean = False) As Boolean
        Return MyBase.Update(Entity.Table, Entity.KeyFields, Entity.DirtyFields)
    End Function
	
	Public function InsertSP(Entity as HorarioEmpleado, Optional ByVal ThrowErr As Boolean = False) as Boolean
        Dim sql As String = "A_HorarioEmpleado_I"
		...
		Try
			...
			par = CType(oParams.Item("@DIASEMANAENTRADA"), IDataParameter)
			par.Value = F_ValueToNull(entity.DIASEMANAENTRADA)
			par = CType(oParams.Item("@DIASEMANASALIDA"), IDataParameter)
			par.Value = F_ValueToNull(entity.DIASEMANASALIDA)
			...
			par = CType(oParams.Item("@INCLUYEFERIADOS"), IDataParameter)
			par.Value = F_BooleanToValue(entity.INCLUYEFERIADOS, par)
	...
	Public function UpdateSP(Entity as HorarioEmpleado, Optional ByVal ThrowErr As Boolean = False) as Boolean
        Dim sql As String = "A_HorarioEmpleado_U"
        Dim oParams As IDataParameterCollection 
        Dim par As IDataParameter
		Dim bOk as Boolean
		Try
	...
	Public Function GetDtTableById(Byval ID as Long, Optional ByVal ThrowErr As Boolean = False) As DataTable
        Dim sql As String = "A_HorarioEmpleadoGetById"
        Dim oParams As IDataParameterCollection
	...
	Public Function GetEntityById(Byval ID as Long, Optional ByVal ThrowErr As Boolean = False) As HorarioEmpleado
	...
	Public Function GetListByEnt(ByVal entity As HorarioEmpleado, Optional ByVal ThrowErr As Boolean = False) As List(Of HorarioEmpleado)
        Dim DtDatos As DataTable
        Dim listEntities As New List(Of HorarioEmpleado)
        Try
	...
	Private Function Make(reader as IDataReader, Optional ByVal ThrowErr As Boolean = False) as HorarioEmpleado
		Dim entity as HorarioEmpleado
		Try
			entity = new HorarioEmpleado
			entity.[New] = false
				...
				if reader("DIASEMANAENTRADA") is System.DbNull.Value then
					entity.DIASEMANAENTRADA = F_Set_nullValue(entity.DIASEMANAENTRADA)
				else
					entity.DIASEMANAENTRADA = CType(reader("DIASEMANAENTRADA"),Integer)
				end if		
				if reader("DIASEMANASALIDA") is System.DbNull.Value then
					entity.DIASEMANASALIDA = F_Set_nullValue(entity.DIASEMANASALIDA)
				else
					entity.DIASEMANASALIDA = CType(reader("DIASEMANASALIDA"),Integer)
				end if		
				...
				if reader("INCLUYEFERIADOS") is System.DbNull.Value then
					entity.INCLUYEFERIADOS = F_Set_nullValue(entity.INCLUYEFERIADOS)
				else
					entity.INCLUYEFERIADOS = F_ValueToBoolean(reader("INCLUYEFERIADOS"))
				end if		
		...
		Catch ex As Exception
			...
			MyBase.ConError = True
			MyBase.Exception = ex
			MyBase.Error = "No se pudo cargar la HorarioEmpleado"
			entity = nothing
			If ThrowErr Then Throw New System.Exception(MyBase.InternalError, MyBase.Exception)
		Finally
	...
	Private Function Make(row as DataRow, Optional ByVal ThrowErr As Boolean = False) as HorarioEmpleado
		Dim entity as HorarioEmpleado
		Try
	...
	if F_Contains(row, "DIASEMANAENTRADA") then
		if row("DIASEMANAENTRADA") is System.DbNull.Value then
			entity.DIASEMANAENTRADA = F_Set_nullValue(entity.DIASEMANAENTRADA)
		else
			entity.DIASEMANAENTRADA = CType(row("DIASEMANAENTRADA"),Integer)
		end if
	end if
	if F_Contains(row, "DIASEMANASALIDA") then
		if row("DIASEMANASALIDA") is System.DbNull.Value then
			entity.DIASEMANASALIDA = F_Set_nullValue(entity.DIASEMANASALIDA)
		else
			entity.DIASEMANASALIDA = CType(row("DIASEMANASALIDA"),Integer)
		end if
	end if
	...
	if F_Contains(row, "INCLUYEFERIADOS") then
		if row("INCLUYEFERIADOS") is System.DbNull.Value then
			entity.INCLUYEFERIADOS = F_Set_nullValue(entity.INCLUYEFERIADOS)
		else
			entity.INCLUYEFERIADOS = F_ValueToBoolean(row("INCLUYEFERIADOS"))
		end if
	end if
...
Public Function GetALL() As List (of HorarioEmpleado)
        Dim sql As String = "A_HORARIOSEMPLEADOSGETALL"
        Dim oParams As IDataParameterCollection
        Dim DtDatos As DataTable
        Dim par As IDataParameter
		Dim listEntities as New List (of HorarioEmpleado)
...
Public Function GetByEmpleado(Byval IDEMPLEADO as Long
        ...
        Dim sql As String = "A_HORARIOSEMPSGETBYEMP"
        Dim oParams As IDataParameterCollection
        Dim DtDatos As DataTable
        Dim par As IDataParameter
		Dim listEntities as New List (of HorarioEmpleado)
