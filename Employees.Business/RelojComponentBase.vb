...
Public Partial Class RelojComponentBase
	...
	Public Overridable Function Validate(Byref entity as Reloj) As Boolean
		Dim bOk As Boolean = True
        Try
			If Not entity.DirtyFields.ContainsKey("CONTROLINGRESO") Then
                entity.DirtyFields.Add("CONTROLINGRESO", entity.CONTROLINGRESO)
            End If
			...
			End if
			if F_IsNullValue(entity.IDUBICACION) Then
				bOk = False 
				Throw New Exception("IDUBICACION required")
			End if
	...
	Public Overridable Function ValidateNew(Byref entity as Reloj) As Boolean
		Dim bOk As Boolean = True
        Try
	...
	Public Overridable Function ValidateDelete(Byref entity as Reloj) As Boolean
		Return False
	End Function


	Public Function Save(Byref col As List (of Reloj), Optional ByVal sp as Boolean = true, Optional byref QInsert As Integer = 0, Optional ByRef QUpdate As Integer = 0, Optional ByVal ThrowErr As Boolean = False) As Boolean
		Dim RelojDal as New RelojData
		Dim bResul As Boolean = True
		Dim strError As String = String.Empty
		Dim flgHuboCambios As Boolean = False
		Try
			RelojDal.BeginTransaction()
			For Each entity As Reloj In col
        	...
        	Catch ex As Exception
				...
				strLog &= "Error Reloj.Save (" & TimeOfDay.ToString("HH:mm:ss tt") & ")" & vbCrLf
        	...
        	Finally
				RelojDal.Desconectar()
        	End Try
        	Return bResul
	End Function


	Public Function Save(Byref entity As Reloj, Optional ByVal sp as Boolean = true, Optional ByVal ThrowErr As Boolean = False) As Boolean
	...
	Private Function Insert(Byref entity as Reloj, Optional ByVal sp as Boolean = true, Optional ByVal ThrowErr As Boolean = False) As Boolean
		Dim oDal As New RelojData
		Dim bOk As Boolean = false
		Try
	...
	Private Function Update(Byref entity as Reloj, Optional ByVal sp as Boolean = true, Optional ByVal ThrowErr As Boolean = False) As Boolean
		Dim oDal As New RelojData
		Dim bOk As Boolean = false
		Try
	...
	Public Function GetEntById(Byval ID as String , Optional ByVal ThrowErr As Boolean = False) As Reloj
		Dim oDal As New RelojData
		Dim entity as Reloj
		Try
	...
	Public Function GetListByEnt(ByVal entity As Reloj, Optional ByVal ThrowErr As Boolean = False) As List(Of Reloj)
        Dim oDal As New RelojData
        Dim listentity As New List(Of Reloj)
        Try
