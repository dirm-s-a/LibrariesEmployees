...
Public Partial Class EventoFichadaComponentBase
	...
	Public Overridable Function Validate(Byref entity as EventoFichada) As Boolean
		Dim bOk As Boolean = True
        Try
			If Not entity.DirtyFields.ContainsKey("INCONSISTENCIA") Then
                entity.DirtyFields.Add("INCONSISTENCIA", entity.INCONSISTENCIA)
            End If
			If Not entity.DirtyFields.ContainsKey("AUSENTE") Then
                entity.DirtyFields.Add("AUSENTE", entity.AUSENTE)
            End If
	...
	Public Overridable Function ValidateNew(Byref entity as EventoFichada) As Boolean
		Dim bOk As Boolean = True
        Try
	...
	Public Overridable Function ValidateDelete(Byref entity as EventoFichada) As Boolean
		Return False
	End Function


	Public Function Save(Byref col As List (of EventoFichada), Optional ByVal sp as Boolean = true, Optional byref QInsert As Integer = 0, Optional ByRef QUpdate As Integer = 0, Optional ByVal ThrowErr As Boolean = False) As Boolean
		Dim EventoFichadaDal as New EventoFichadaData
		Dim bResul As Boolean = True
		Dim strError As String = String.Empty
		Dim flgHuboCambios As Boolean = False
		Try
			EventoFichadaDal.BeginTransaction()
			For Each entity As EventoFichada In col
			...
			If Not flgHuboCambios Then
				EventoFichadaDal.CancelTransaction()
				If strError = String.Empty Then strError = "No hay nada para guardar."
				bResul = False
			End If
			If bResul = True Then
				EventoFichadaDal.CommitTransaction()
			Else
        	...
        	Catch ex As Exception
				...
				strLog &= "Error EventoFichada.Save (" & TimeOfDay.ToString("HH:mm:ss tt") & ")" & vbCrLf
        	...
        	Finally
				EventoFichadaDal.Desconectar()
        	End Try
        	Return bResul
	End Function


	Public Function Save(Byref entity As EventoFichada, Optional ByVal sp as Boolean = true, Optional ByVal ThrowErr As Boolean = False) As Boolean
	...
	Private Function Insert(Byref entity as EventoFichada, Optional ByVal sp as Boolean = true, Optional ByVal ThrowErr As Boolean = False) As Boolean
	...
	Private Function Update(Byref entity as EventoFichada, Optional ByVal sp as Boolean = true, Optional ByVal ThrowErr As Boolean = False) As Boolean
		Dim oDal As New EventoFichadaData
		Dim bOk As Boolean = false
		Try
	...
	Public Function GetEntById(Byval ID as Long , Optional ByVal ThrowErr As Boolean = False) As EventoFichada
		Dim oDal As New EventoFichadaData
	...
	Public Function GetListByEnt(ByVal entity As EventoFichada, Optional ByVal ThrowErr As Boolean = False) As List(Of EventoFichada)
        Dim oDal As New EventoFichadaData
        Dim listentity As New List(Of EventoFichada)
        Try
...
Byval IDEMPLEADO as Integer, Byval FECHADESDE as Date, Byval FECHAHASTA as Date
) As Boolean
		Dim oDal As New EventoFichadaData
		Try
	        Return oDal.Delete(
IDEMPLEADO, FECHADESDE, FECHAHASTA
)
		Catch ex As Exception
            MyBase.Exception = ex
            MyBase.InternalError = ex.Message
...
ID
	...
	Public Function GetTodoEntreFechasEntrada(
Byval FECHADESDE as Date, Byval FECHAHASTA as Date
) As List (of EventoFichada)
		Dim oDal As New EventoFichadaData
		Try
	        Return oDal.GetTodoEntreFechasEntrada(
FECHADESDE, FECHAHASTA
	...
	Public Function GetTodoEntreFechaseEntradaGrupo(
Byval FECHADESDE as Date, Byval FECHAHASTA as Date, Byval IDGRUPO as Integer
) As List (of EventoFichada)
		Dim oDal As New EventoFichadaData
		Try
	        Return oDal.GetTodoEntreFechaseEntradaGrupo(
FECHADESDE, FECHAHASTA, IDGRUPO
	...
	Public Function GetByIdLiquidacion(
Byval IDLIQUIDACION as Integer
) As List (of EventoFichada)
		Dim oDal As New EventoFichadaData
		Try
	        Return oDal.GetByIdLiquidacion(
IDLIQUIDACION
)
		Catch ex As Exception
            MyBase.Exception = ex
